...
# ------------------ Parsing ------------------
KV_RE = re.compile(
    r"(lead owner|owner|name|title|email|company|company name|phone|phone number|first_name|last_name|"
    r"street|address|city|state|zip|zip code|postal code|country|notes)"
    r"\s*[:=]\s*([^;\n]+)",
    re.IGNORECASE,
)
...
def parse_lead(text: str):
    ...
    lead_owner = data.get("lead owner") or data.get("owner") or ""
...
async def zoho_create_lead(lead: dict):
    ...
    lead_record = {
        ...
        "Description": (
            "Captured from Webex."
            + (("\n\nNotes: " + lead.get("notes", "")) if lead.get("notes") else "")
            + "\n\nRaw: " + (lead.get("raw", "") or "")
        ),
